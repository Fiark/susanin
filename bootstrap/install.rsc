# Susanin v0.11.5 credentialless bootstrap
...
/system script add name="susanin-bootstrap-worker" policy=read,write,test,ftp,policy,password dont-require-permissions=yes source={
    ...
    :if ([:len [/container find where name="susanin-controller"]] > 0) do={
        ...
        :if ($susaninBootstrapRoot = "/susanin-controller-v0115") do={
            ...
            :if ($susaninBootstrapTag != "0.11.5") do={
                /system scheduler disable [find where name="susanin-bootstrap-worker"]
                :log error ("SUSANIN: extracted image tag mismatch, expected 0.11.5 got " . $susaninBootstrapTag)
                :exit
            }
    ...
    :onerror susaninContainerAddError in={
        /container add file=susanin.tar interface=veth-susanin root-dir=/susanin-controller-v0115 mountlists=susanin-secret,susanin-data name=susanin-controller logging=yes start-on-boot=yes cmd=daemon
    } do={
